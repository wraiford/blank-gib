            ...
            infos: [
                {
                    ...
                    storeNames: [ARMY_STORE, ZERO_SPACE_ID],
                }
            ]
        });
...
export function getComponentCtorArg(): IbGibDynamicComponentMetaCtorOpts {
    const lc = `[${getComponentCtorArg.name}]`;
    try {
        if (logalot) { console.log(`${lc} starting... (I: e6f0689e88c451c178bb4e3bd441d825)`); }

        const fnGetMetaspace = async () => { return await getGlobalMetaspace_waitIfNeeded(); };
        ...
        return {
            fnGetMetaspace,
            bootstrapPromise: bootstrapPromiseWrapper,
        };
    } catch (error) {
        console.error(`${lc} ${extractErrorMsg(error)}`);
        throw error;
